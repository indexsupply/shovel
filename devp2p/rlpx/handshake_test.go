...
func TestSendAuth(t *testing.T) {
	...
	initPrvKey := secp256k1.PrivKeyFromBytes(eth.DecodeHexString(tv["initiator_private_key"]))
	initEphPrvKey := secp256k1.PrivKeyFromBytes(eth.DecodeHexString(tv["initiator_ephemeral_private_key"]))
	initNonce := eth.DecodeHexString(tv["initiator_nonce"])
	if len(initNonce) != 32 {
		t.Errorf("initNonce is not 32 bytes")
	}
	var nonce [32]byte
	copy(nonce[:], initNonce)
	recPrvKey := secp256k1.PrivKeyFromBytes(eth.DecodeHexString(tv["receiver_private_key"]))
	h := &handshake{
		isInitiator: true,
	...
	expectedAuthMsg := eth.DecodeHexString(tv["auth_plaintext"])[65:]
	...
	recPubKey := recPrvKey.PubKey().SerializeUncompressed()
	eciesKey, err := eciesgo.NewPublicKeyFromBytes(recPubKey[:])
	tc.NoErr(t, err)
	cipher, err := eciesgo.Encrypt(eciesKey, authMsg)
	tc.NoErr(t, err)
